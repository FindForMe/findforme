...
function ajaxSend(url, params, method, caller,showUrl, showBeforeSend, showComplete ){
	/* xhr - xmlHttpResponse Object */
	
	if(typeof showBeforeSend == 'undefined' ){
		var showBeforeSend = false;
	}
	if(typeof showComplete == 'undefined' ){
	...
	$.ajax({
		...
		success : function(result , status, xhr){
			...
			ajaxSuccess(result, caller);
		},
		error : function(xhr, status, error){
			if(showUrl){
				console.log('url : ',this.url);
			}
			ajaxError(xhr.status, caller);
		}
	});
}
